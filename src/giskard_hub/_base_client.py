...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        if max_retries is None:  # pyright: ignore[reportUnnecessaryComparison]
            raise TypeError(
                "max_retries cannot be None. If you want to disable retries, pass `0`; if you want unlimited retries, pass `math.inf` or a very high number; if you want the default behavior, pass `giskard_hub.DEFAULT_MAX_RETRIES`"
            )
    ...
    def _build_headers(self, options: FinalRequestOptions, *, retries_taken: int = 0) -> httpx.Headers:
        ...
        if "x-giskard-retry-count" not in lower_custom_headers:
            headers["x-giskard-retry-count"] = str(retries_taken)
        if "x-giskard-read-timeout" not in lower_custom_headers:
            timeout = self.timeout if isinstance(options.timeout, NotGiven) else options.timeout
            if isinstance(timeout, Timeout):
                timeout = timeout.read
            if timeout is not None:
                headers["x-giskard-read-timeout"] = str(timeout)
    ...
    def _idempotency_key(self) -> str:
        return f"giskard-python-retry-{uuid.uuid4()}"
...
def platform_headers(version: str, *, platform: Platform | None) -> dict[str, str]:
    return {
        "X-Giskard-Lang": "python",
        "X-Giskard-Package-Version": version,
        "X-Giskard-OS": str(platform or get_platform()),
        "X-Giskard-Arch": str(get_architecture()),
        "X-Giskard-Runtime": get_python_runtime(),
        "X-Giskard-Runtime-Version": get_python_version(),
    }
...
Arch = OtherArch | Literal["x32", "x64", "arm", "arm64", "unknown"]
